...
def test_create_project_end_to_end_via_http_with_demo_llm(tmp_path):
    """The one test that exercises the full HTTP path (POST /projects ->
    run_organization -> ProjectResponse), using the same demo LLM provider
    documented for cost-free verification. The underlying pipeline is
    already covered thoroughly by test_demo_client.py; this just proves
    the API layer wires it up and shapes the response correctly."""
    db_url = f"sqlite:///{tmp_path}/test.db"
    long_term = LongTermMemory(database_url=db_url)
    long_term.init_schema()
    ...
    try:
        response = _client().post(
            "/projects", json={"goal": "Launch a customer feedback widget"}
        )
        assert response.status_code == 200
        body = response.json()
        ...
        entries = _client().get("/memory-entries").json()
